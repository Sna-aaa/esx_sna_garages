...
function OpenCompanyGarageMenu(company)
	local elements = {}
	...
	for k,v in pairs(Config.Companies) do
		if v.UseBlips then
			if ESX.PlayerData.job and ESX.PlayerData.job.name == v.Job then
				local blip = AddBlipForCoord(v.Locations.Marker)
				SetBlipSprite (blip, v.BlipsGStyle.Sprite)
				SetBlipColour (blip, v.BlipsGStyle.Color)
				SetBlipDisplay(blip, v.BlipsGStyle.Display)
				SetBlipScale  (blip, v.BlipsGStyle.Scale)
				SetBlipAsShortRange(blip, true)

				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString(_U('blip_'..v.Job..'garage'))
				EndTextCommandSetBlipName(blip)
		...
		if v.UsePounds then
			if ESX.PlayerData.job and ESX.PlayerData.job.name == v.Job then
				local blip = AddBlipForCoord(v.Pound.Marker)
				SetBlipSprite (blip, v.BlipsPStyle.Sprite)
				SetBlipColour (blip, v.BlipsPStyle.Color)
				SetBlipDisplay(blip, v.BlipsPStyle.Display)
				SetBlipScale  (blip, v.BlipsPStyle.Scale)
				SetBlipAsShortRange(blip, true)

				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString(_U('blip_'..v.Job..'impound'))
				EndTextCommandSetBlipName(blip)
				table.insert(JobBlips, blip)
			end
		end
...
function OpenCarGarageMenu()
	...
	if NoCars and not Config.Main.TruckShop then
		ESX.UI.Menu.CloseAll()
		ESX.ShowNotification(_U('garage_no_veh'))
	else
		ESX.UI.Menu.CloseAll()
